...
var (
	chatHistory []map[string]string // Stores the conversation history
	resetTimer  *time.Timer         // Timer for resetting chat history
	timeout     = time.Hour          // 1 hour timeout duration
)

func resetHistory() {
	log.Println("Chat history reset due to inactivity.")
	chatHistory = []map[string]string{}
}

func restartTimer() {
	if resetTimer != nil {
		resetTimer.Stop() // Stop the previous timer
	}
	resetTimer = time.AfterFunc(timeout, resetHistory) // Start a new 1-hour timer
}
